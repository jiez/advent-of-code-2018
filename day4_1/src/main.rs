...
*/
    ...
    let mut most_sleep_minutes = 0;
    for (id, sleep_minutes) in &all_sleep_minutes {
        let mut total_sleep_minutes = 0;
        for i in 0..60 {
            total_sleep_minutes += sleep_minutes[i];
        }
        println!("{} {}", id, total_sleep_minutes);
        if total_sleep_minutes > most_sleep_minutes {
            laziest_id = *id;
            most_sleep_minutes = total_sleep_minutes;
        }
    }

    let mut laziest_minute = 0;
    let mut max_count = 0;
    if let Some(sleep_minutes) = all_sleep_minutes.get(&laziest_id) {
        for i in 0..60 {
